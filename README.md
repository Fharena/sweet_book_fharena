...
한 문장 설명:
여행 사진을 날짜와 장소 기준으로 자동 정리해 포토북 초안을 만들고 Sweetbook 주문까지 연결하는 여행 포토북 웹앱입니다.

타겟 고객:
- 여행 사진은 많이 찍지만 직접 앨범을 정리할 시간은 부족한 사용자
- 가족, 연인, 친구와 다녀온 여행을 기념 포토북으로 남기고 싶은 사용자
- 사진 정리와 편집보다 빠르게 “한 권의 결과물”을 얻고 싶은 사용자

주요 기능:
- 여행 제목과 기간 설정
- 사진 업로드 및 EXIF 촬영시간/GPS 읽기
- 날짜별, 장소별 자동 그룹핑
- 날짜/위치 수동 보정 및 다중 선택 편집
- 포토북 포맷 3종 미리보기
- Sweetbook 테스트 책 생성 및 주문 요청
- 웹훅 운영 패널과 receipt 추적
...
production 확인이 필요할 때는 아래 순서로 실행합니다.
...
| API | 용도 |
| --- | --- |
| `POST /v1/books` | 새 여행 포토북 draft 생성 |
| `POST /v1/books/{bookUid}/photos` | 여행 사진을 Sweetbook 서버 파일로 업로드 |
| `POST /v1/books/{bookUid}/cover` | 포토북 표지 생성 |
| `POST /v1/books/{bookUid}/contents` | 챕터 divider, 본문, publish 페이지 생성 |
| `POST /v1/books/{bookUid}/finalization` | 책 최종화 |
| `GET /v1/book-specs` | 판형 정보와 최소/증분 페이지 규칙 조회 |
| `POST /v1/orders` | 최종화된 책 주문 생성 |
| `GET /v1/webhooks/config` | 웹훅 설정 상태 조회 |
| `PUT /v1/webhooks/config` | 웹훅 URL 및 이벤트 등록 |
| `DELETE /v1/webhooks/config` | 웹훅 설정 해제 |
...
| AI 도구 | 활용 내용 |
| --- | --- |
| Codex | 전체 풀스택 구현, Sweetbook API 연동, 스튜디오 플로우 및 리뷰 UX 수정 |
| ChatGPT / Codex 계열 모델 | 디버깅 아이디어 검토, 요구사항 재점검, 문서 정리 |
| Claude / Sonnet 계열 | 모바일 업로드 실패 원인 가설 검토 및 이벤트 구조 점검 |
| Google Stitch | 초기 모바일 UI 방향과 step별 레이아웃 레퍼런스 생성 |

## 설계 의도

왜 이 서비스를 선택했는지:
여행 포토북은 과제 문서의 예시와도 잘 맞고, `사진 업로드 -> 자동 정리 -> 책 생성 -> 주문` 흐름이 가장 자연스럽게 이어지는 주제라고 판단했습니다.

비즈니스 가능성:
- 여행 후 남는 사진 정리 피로를 줄여주는 명확한 문제 해결이 있습니다.
- 포토북은 선물, 기념, 개인 아카이빙 수요가 꾸준합니다.
- 추후 여행 상품 제휴, AI 여행 요약, 공동 편집, 시즌별 템플릿 확장으로 확장성이 있습니다.

더 시간이 있었다면 추가했을 기능:
- 장소명을 더 자연스럽게 붙이는 고도화된 reverse geocoding 및 POI 추천
- Gemini 기반 여행 요약 문구/챕터 카피 자동 생성
- 포토북 페이지 순서 재배치와 세부 편집기
- 공유 링크 및 공동 여행자 초대
- 주문 상태 타임라인과 웹훅 운영 화면의 통합
